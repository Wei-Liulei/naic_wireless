...
class DeuantizationLayer(tf.keras.layers.Layer):
    ...
    def get_config(self):
        ...
        return base_config
"""    
class SELayer(tf.nn.Module):
    def __init__(self, channel, reduction=16):
        super(SELayer, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.fc = tf.nn.Sequential(
            tf.nn.Linear(channel, channel // reduction, bias=True),
            tf.nn.LeakyReLU(inplace=True),
            tf.nn.Linear(channel // reduction, channel, bias=True),
            tf.nn.Sigmoid()
        )
...
def Decoder(x,feedback_bits):
    ...
    for i in range(3):
        ...
        x = tf.nn.swish(x)
        x = layers.Conv2D(16, 7, padding='SAME')(x)
        x = tf.nn.swish(x)
        x = layers.Conv2D(2, 7, padding='SAME')(x)
        x = tf.nn.swish(x)
...
if __name__=='__main__':
    num0 = tf.constant([[1.5, 2.5],
                        [3, 4],
                        [5, 6]], dtype=tf.float16)
    b = Num2Bit(tf.constant(num0), 4)
    num = Bit2Num(b, 4)
